...
async function generateSubQuery(payload, user) {
   ...
   * @todo: Implement minimum count in front end if required 
   */
  ...
  if ((payload.dictionary.filter(obj => obj.map_name === 'time_slots_map')).length > 0){
    ...
    'count(s' + payload.sensor_id + `) AS selectedSensor_Count FROM user_sensors WHERE workspace_id = 1 AND user_id = '` + user.id + `' \ ` + 
