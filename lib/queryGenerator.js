...
async function generateSubQuery(payload, user) {
   ...
   * @todo: Remove hard-coded workspace ID 
   */
  return 'SELECT ' + arr.join(', ') + `, selectedSensor, timeStamp \
  FROM ( \
     SELECT s${payload.sensor_id} as selectedSensor, timestamp AS timeStamp FROM user_sensors \
  ...
  // return `SELECT case WHEN foo.selectedSensor BETWEEN 0 and 10 THEN 'Null' WHEN foo.selectedSensor BETWEEN 10 and 110 THEN '1' \
  //   WHEN foo.selectedSensor BETWEEN 110 and 210 THEN '2' WHEN foo.selectedSensor BETWEEN 210 and 310 THEN '3' WHEN foo.selectedSensor \
  //   BETWEEN 310 and 410 THEN '4' WHEN foo.selectedSensor BETWEEN 410 and 510 THEN '5' WHEN foo.selectedSensor BETWEEN 510 and 610 THEN \
  //   '6' WHEN foo.selectedSensor BETWEEN 610 and 710 THEN '7' WHEN foo.selectedSensor BETWEEN 710 and 810 THEN '8' WHEN foo.selectedSensor \
  //   BETWEEN 810 and 910 THEN '9' end AS activity_range, selectedSensor, timeStamp FROM (SELECT s${payload.sensor_id} as selectedSensor, timestamp AS timeStamp \
  //   FROM user_sensors WHERE workspace_id = 2 AND s4 > 100 AND user_id = '${user.id}' AND 1=1   ) AS foo;`
  }
