    ...
    function (cb) {
      switch (userType) {
        case Config.APP_CONSTANTS.DATABASE.USER_ROLES.RESEARCHER:
          Service.ResearcherService.getRecord(criteria, {}, { lean: true }, function (err, dataAry) {
            if (err) {
              cb(err)
            } else {
...
var setTokenInDB = function (userId, userType, tokenToSave, callback) {
  ...
  async.series([
    function (cb) {
      switch (userType) {
        case Config.APP_CONSTANTS.DATABASE.USER_ROLES.RESEARCHER:
          Service.ResearcherService.updateRecord(criteria, setQuery, { new: true }, function (err, dataAry) {
            if (err) {
              cb(err)
            } else {
