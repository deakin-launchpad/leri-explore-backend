...
/**
 * Description: Method to create visualization query.  
 * Author: Somanshu Kalra
 */
module.exports.getVisualizationData = async (user) => {
 try {
   const query = `SELECT timestamp, s1, s2, s3, s4, s5, s6, s7,s8,s9 FROM user_sensors WHERE user_id = '${user.id}'`;
   return [null, query];
 } catch (error) {
   return [error, null];
 } 
