  ...
  return new Promise((resolve, reject) => {
    fs.readFile(headerFilePath, "utf8", (err, headerFile) => {
      ...
      bodyFileData = bodyFileData.map((row, i) => {
        return createObjFromRow(row, fDate, totalS, 'seconds', i, params.id, serialNo, mode)
      })
...
function createObjFromRow(row, initialMoment, amountToAdd, units, index, workspace_id, serialNo, mode) {
  let binary_mode = parseInt(mode).toString(2).split('');
  return {
    ...
    s1: row[0],
    s2: binary_mode[3] === '1' ? row[1] : 0,
    s3: binary_mode[2] === '1' ? row[2] : 0,
    s4: binary_mode[5] === '1' ? row[3] : 0,
    s5: binary_mode[4] === '1' ? row[4] : 0,
    s6: binary_mode[1] === '1' ? row[5] : 0,
    s7: binary_mode[0] === '1' ? row[6] : 0, 
    s8: binary_mode[0] === '1' ? row[7] : 0,
    s9: binary_mode[0] === '1' ? row[8]: 0
  }
}
