...
async function processHeader(headerFilePath, bodyFileData) {
  return new Promise((resolve, reject) => {
...
async function findFirstValidRecord() { }

async function deleteTempDirr(dirr) {
  return new Promise((resolve, reject) => {
    exec(`rm -rf ${dirr}`,
      function (error, stdout, stderr) {
        if (error || stderr) return reject(error || stderr)
...
const processMultipleFiles = async function (payload, cb) {
  const root = 'uploads'
  let bodyFileData = []
  ...
  try {
    for (let i = 0; i < payload.file.length; ++i) {
      ...
      const data3 = await processHeader(headerFilePath, tempBodyFileData)
...
module.exports.processFile = async function (payload, cb) {
  if (Array.isArray(payload.file) && payload.file.length > 1) return processMultipleFiles(payload, cb)
  ...
  try {
    ...
    const data3 = await processHeader(headerFilePath, bodyFileData)
